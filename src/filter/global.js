...
export default {
    valuefilter:function(value,valuePipe){
      try {
        ...
        } else if(valuePipe.scale) {
          return this.SubstituteNum(value, valuePipe.scale)
        } else if(valuePipe.hidePhoneNo){
          return this.SubstitutePhoneNo(value, valuePipe.hidePhoneNo)
        } else if(valuePipe.hideName) {
          return this.SubstituteName(value, valuePipe.hideName)
        } else {
          return value
        }
      } catch (err) {
  ...
  SubstitutePhoneNo(value, hidePhoneNo) {
    if (!value) {
      return '---'
    }else{
  ...
  SubstituteName(value, hideName) {
    if (!value) {
      return '---'
    }else{
      return value.slice(0, 1) + '*'.repeat(value.slice(1).length)
    } 
  }
}
